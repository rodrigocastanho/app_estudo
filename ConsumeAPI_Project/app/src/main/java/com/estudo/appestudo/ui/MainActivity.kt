...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btTranslateApi.setOnClickListener {
            viewModel.fetchWordAndDefinitionsApi(binding.edText.text.toString())
        }
        ...
        lifecycleScope.launch {
            repeatOnLifecycle(STARTED) {
                ...
                launch {
                    viewModel.statusExibirMessageError.collect { status ->
                        if (status) {
                            viewModel.statusMessageError.collect { message ->
